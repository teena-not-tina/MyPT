...
const FoodDetectionApp = () => {
  ...
  const analyzeImageAdvanced = async (imageIndex) => {
    ...
    try {
      setProcessingStep(`이미지 ${imageIndex + 1} 4단계 추론 중...`);
      setStatusMessage(`이미지 ${imageIndex + 1} 4단계 추론 중...`);
      ...
      try {
        ocrResult = await apiService.performOCR(image.dataUrl);
        setOcrResults(prev => ({
          ...prev,
          [image.id]: ocrResult
        }));
      ...
      if (hasOcrText) {
        ...
        try {
          const foodName = await callGeminiAPIWithAdvancedInference(ocrResult.text);
          if (foodName) {
            setGeminiResults(prev => ({
              ...prev,
              [image.id]: {
        ...
        try {
          for (let detection of detectionResult.detections.filter(d => d.confidence >= 0.5)) {
            ...
            const translatedName = await geminiService.translateDetectionResult(detection.class);
            if (translatedName) {
              finalIngredients.push({
                name: translatedName,
                quantity: 1,
                  ...
                  {activeTab === 'gemini' && (
                    <div>
                      ...
                      {images.length > 0 && selectedImageIndex >= 0 && geminiResults[images[selectedImageIndex]?.id] ? (
                        ...
                        <p className="text-xs md:text-sm text-gray-500 text-center py-6">이미지를 선택하고 4단계 추론을 진행해주세요.</p>
                      )}
                    </div>
                  )}
                </div>
