...
async def startup_event():
    ...
    try:
        # 비동기 모델 로딩 시도
        from services.model_loader import load_ensemble_models_async
        models = await load_ensemble_models_async()
        models_count = len(models)
        
        # DB 연결 확인
        db_connected = fridge_collection is not None
        
        init_time = time.time() - start_time
        
        print(f"✅ 초기화 완료 ({init_time:.2f}초)")
        print(f"   Models: {models_count}/3 loaded")
        print(f"   Database: {'✅' if db_connected else '❌'}")
        print(f"   Ensemble: {'Ready' if models_count >= 2 else 'Limited'}")
        
    except Exception as e:
        print(f"⚠️ 비동기 초기화 실패, 동기 방식으로 fallback: {e}")
