...
async def save_fridge_data(fridge_data: FridgeData):
    """냉장고 데이터 저장 API - main.py에서 그대로 이동"""
    from main import fridge_collection, client, DB_NAME, COLLECTION_NAME
...
async def save_simple_fridge_data(fridge_data: SimpleFridgeData):
    """간단한 냉장고 데이터 저장 API - main.py에서 그대로 이동"""
    from main import fridge_collection, client, DB_NAME, COLLECTION_NAME
...
async def load_fridge_data(user_id: str):
    """냉장고 데이터 로드 API - main.py에서 그대로 이동"""
    from main import fridge_collection, DB_NAME, COLLECTION_NAME
...
async def load_simple_fridge_data(user_id: str):
    """간단한 냉장고 데이터 로드 API - main.py에서 그대로 이동"""
    from main import fridge_collection, DB_NAME, COLLECTION_NAME
...
async def load_v3_fridge_data(user_id: str):
    """V3 냉장고 데이터 로드 API - main.py에서 그대로 이동"""
    from main import fridge_collection, db, DB_NAME, COLLECTION_NAME, convert_v3_to_current_format
...
async def migrate_v3_to_current(user_id: str):
    """V3 데이터를 현재 형식으로 마이그레이션 API - main.py에서 그대로 이동"""
    from main import fridge_collection, DB_NAME, COLLECTION_NAME
