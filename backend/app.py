...
# --- 설정값 ---
# OPENAI_API_KEY는 .env 파일에서 로드됩니다.
...
def chat_and_generate():
    ...
    # user_openai_api_key는 더 이상 프론트엔드에서 오지 않으므로 사용하지 않거나,
    # 백엔드 환경 변수가 없는 경우를 대비한 대체 옵션으로만 둡니다.
    # user_openai_api_key = data.get('api_key')

    print("\n--- Request received ---")
    print(f"User message: {user_meal_message}")
    # print(f"API Key received (from frontend): {'Present' if user_openai_api_key else 'Not Present'}") # 이 줄은 이제 필요 없습니다.
    print(f"Using OPENAI_API_KEY from .env: {OPENAI_API_KEY is not None}")

    if not user_meal_message:
        return jsonify({"error": "메시지를 입력해주세요."}), 400

    # API 키 사용 로직 변경: 환경 변수에서 먼저 가져옵니다.
    api_key_to_use = OPENAI_API_KEY
    if not api_key_to_use:
        return jsonify({"error": "OpenAI API 키가 서버에 설정되지 않았습니다."}), 400

    if not COMFYUI_WORKFLOW:
        return jsonify({"error": "ComfyUI 워크플로우를 로드할 수 없습니다. comfyui_workflow.json 파일을 확인하세요."}), 500

    # 결정된 API 키로 OpenAI 클라이언트를 초기화합니다.
