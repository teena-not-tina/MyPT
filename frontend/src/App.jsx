...
function App() {
  ...
  const [apiKey, setApiKey] = useState(null); // OpenAI API 키
  ...
  const handleSetApiKey = (key) => {
    ...
    setApiKey(key);
    setMessages((prev) => [...prev, { sender: 'bot', text: 'API 키가 설정되었습니다. 이제 음식을 알려주세요!' }]);
  };

  const sendMessage = async () => {
    if (!userInput.trim() || !apiKey) {
      alert('음식을 입력하거나 API 키를 설정해주세요.');
      return;
    }
        ...
        {!apiKey && (
          <div className="api-key-input">
            <input
              ...
              onBlur={(e) => handleSetApiKey(e.target.value)}
              onKeyPress={(e) => e.key === 'Enter' && handleSetApiKey(e.target.value)} // 엔터 키로도 API 키 설정 가능
            />
            <button onClick={() => handleSetApiKey(document.querySelector('.api-key-input input').value)}>
              API 키 설정
            </button>
          </div>
        )}
